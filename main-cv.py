...
def run(args: DictConfig):
        ...
        for epoch in range(args.num_epochs):
            ...
            if args.use_wandb:
                wandb.log({
                    ...
                    f'loss/validation/fold-{fold}': val_loss,
                    f'acc/validation/fold-{fold}': val_acc,
                    'epoch': epoch
                })
            ...
            if args.use_wandb:
                wandb.log({f'acc/validation/fold-{fold}': max_val_acc})
    ...
    if not args.dry_run:
        ...
        # 平均化された予測結果を保存
        submission_file_path = os.path.join(save_folder_name, "submission.npy")
